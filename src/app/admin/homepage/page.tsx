    ...
    >
      <form onSubmit={handleSubmit} className="mx-auto max-w-2xl space-y-10">
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className={labelClass}>Featured Installations — Title</label>
              <input
                type="text"
                value={content.visualProof.featuredInstallations.title}
                onChange={(e) =>
                  setContent((c) => ({
                    ...c!,
                    visualProof: {
                      ...
                      featuredInstallations: {
                        ...c!.visualProof.featuredInstallations,
            ...
            <div>
              <label className={labelClass}>Featured Installations — Description</label>
              <input
                type="text"
                value={content.visualProof.featuredInstallations.description}
                onChange={(e) =>
                  setContent((c) => ({
                    ...c!,
                    visualProof: {
          ...
          <div>
            <label className={labelClass}>Featured Installations — Images</label>
            <p className="mt-1 text-xs text-foreground/60">
              Add images with URL, alt text, and optional label. Upload via Storage, then paste the URL.
            </p>
            <div className="mt-2 space-y-3">
              {content.visualProof.featuredInstallations.images.map((img, i) => (
                <ImageRow
                  key={i}
                  img={img}
                  showLabel
                  onChange={(updated) =>
                    updateImageArray("featuredInstallations", (imgs) => {
                      const next = [...imgs];
                      next[i] = updated;
                      return next;
                    })
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className={labelClass}>Product Highlights — Title</label>
              <input
                type="text"
                value={content.visualProof.productHighlights.title}
                onChange={(e) =>
                  setContent((c) => ({
                    ...c!,
                    visualProof: {
                      ...c!.visualProof,
                      productHighlights: {
                        ...c!.visualProof.productHighlights,
                        title: e.target.value,
                      },
                    },
                  }))
            ...
            <div>
              <label className={labelClass}>Product Highlights — Description</label>
              <input
                type="text"
                value={content.visualProof.productHighlights.description}
                onChange={(e) =>
                  setContent((c) => ({
                    ...c!,
                    visualProof: {
          ...
          <div>
            <label className={labelClass}>Product Highlights — Images</label>
            <p className="mt-1 text-xs text-foreground/60">
              Upload an image or paste a URL. Add a Link URL to make the image clickable.
            </p>
            <div className="mt-2 space-y-3">
              {content.visualProof.productHighlights.images.map((img, i) => (
                <ImageRow
                  key={i}
                  img={img}
                  showLabel
                  ...
                  getHeaders={getHeaders}
                  onChange={(updated) =>
                    updateImageArray("productHighlights", (imgs) => {
                      const next = [...imgs];
                      next[i] = updated;
                      return next;
                    })
